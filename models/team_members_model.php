...
class TeamMembers implements DatabaseObject{
    ...
    public function __construct($pk_team_member_id, $since, $team_id , $team_name, $user_id,$username,$fullname,$type){
    ...
    public static function getObjList($params){
        ...
        $query='SELECT tm.*,u.fullname,u.username FROM minyy.team_members as tm LEFT JOIN minyy.users as u on tm.user_id = u.pk_user_id';
    ...
    public static function getObj($id){
        try{
            $db = Db::getInstance();
            $id = intval($id);
            $req = $db->prepare('SELECT tm.*,u.fullname,u.username FROM minyy.team_members as tm LEFT JOIN minyy.users as u on tm.user_id = u.pk_user_id WHERE t.pk_team_member_id = :pk_team_member_id');
            $req->execute(array('pk_team_member_id' => $id));
            $obj = $req->fetch();
            if($obj){
    ...
    public static function getTeamMembers($teamId,$params){
        ...
        $query='SELECT TM.*, U.username, U.fullname, (SELECT name FROM teams WHERE pk_team_id=TM.team_id) AS name FROM team_members AS TM LEFT JOIN users AS U ON TM.user_id=U.pk_user_id WHERE TM.team_id=:teamId';

        if(!empty($params['search_term'])){
            $query.=" AND lower(concat(pk_team_member_id, '', since, '', team_id , '', user_id, '', type )) LIKE :search_term";
        }
        $query.=" ORDER BY $order_by $order_dir LIMIT :offset , :limit";
