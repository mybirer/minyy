<?php
    class Posts implements DatabaseObject {
        ...
        public function __construct($pk_post_id,$created_by,$author_name,$created_at,$post_title,$post_alias,$post_content,$post_status,$comment_status,$modified_by,$modified_date,$guid,$post_type,$featured_image,$comment_count,$post_params){
        ...
        public static function insert($params){
            $return=[];
            if ($params === null || empty($params['created_by']) || empty($params['post_title']) || empty($params['post_alias']) || empty($params['post_content']) || empty($params['post_status']) || empty($params['comment_status']) || empty($params['post_type'])) {
                $return['status']=false;
                ...
                //if not, save it
                $req = $db->prepare('INSERT INTO posts (created_by,created_at,post_title,post_alias,post_content,post_status,comment_status,guid,post_type,featured_image,post_params) VALUES (:created_by , NOW() , :post_title , :post_alias , :post_content , :post_status , :comment_status , :guid , :post_type , :featured_image , :post_params )');
        ...
        public static function update($id,$params){
            $return=[];
            if ($params === null || empty($params['modified_by']) || empty($params['post_title']) || empty($params['post_alias']) || empty($params['post_content']) || empty($params['post_status']) || empty($params['comment_status']) || empty($params['post_type'])) {
                $return['status']=false;
                $return['message']=T::__("Please fill all required fields!",true);
                return $return;
            }
                ...
                //if not, save it
                $req = $db->prepare('UPDATE posts SET post_title = :post_title ,post_alias = :post_alias ,post_content = :post_content ,post_status = :post_status ,comment_status = :comment_status ,modified_by = :modified_by ,modified_date = NOW() ,post_type = :post_type ,featured_image = :featured_image , post_params = :post_params WHERE pk_post_id = :id');
                $res=$req->execute(array(
                    'post_title' => htmlspecialchars($params['post_title']),
                    'post_alias' => htmlspecialchars($params['post_alias']),
                    'post_content' => htmlspecialchars($params['post_content']),
        ...
        public static function getObjList($params){
            ...
            if(empty($params["post_type"]) || ($params["post_type"]!='post' && $params["post_type"]!='page'))
                return $list;
            ...
            $order_columns=array("pk_post_id","created_by", "created_at", "modified_by", "modified_date", "comment_count");
            ...
            $query="SELECT p.pk_post_id,p.created_by,u.username,p.created_at,p.post_title,p.post_alias,p.post_status,p.comment_status,p.modified_by,p.modified_date,p.guid,p.post_type,p.featured_image,p.comment_count,p.post_params FROM minyy.posts as p LEFT JOIN minyy.users as u on p.created_by=u.pk_user_id WHERE p.post_type=:post_type";
            if(!empty($params['search_term'])){
                $query.=" AND lower(concat(pk_post_id, '', created_by, '', post_title , '', post_content, '', created_at, '', modified_by, '', modified_date , '', comment_count)) LIKE :search_term";
            }
            $query.=" ORDER BY $order_by $order_dir LIMIT :offset , :limit";
            $db = Db::getInstance();
            $req = $db->prepare($query);
            $req->bindValue(':post_type', $post_type, PDO::PARAM_STR);
        ...
        public static function getObj($id){
            try{
                $db = Db::getInstance();
                $id = intval($id);
                $req = $db->prepare('SELECT p.* , u.username FROM minyy.posts as p LEFT JOIN minyy.users as u on p.created_by=u.pk_user_id WHERE p.pk_post_id = :pk_post_id');
                $req->execute(array('pk_post_id' => $id));
                $obj = $req->fetch();
                if($obj){
        ...
        public static function getTotal($search_term) {
            ...
            if(!empty($search_term))
                $query.=" WHERE lower(concat(pk_post_id, '', created_by, '', post_title , '', post_content, '', created_at, '', modified_by, '', modified_date, '', post_type , '', comment_count)) LIKE :search_term";
            $db = Db::getInstance();
            $req = $db->prepare($query);
            if(!empty($search_term))
